...
def load_features(ID, data_path, mode):
    if mode == "fast":
        ...
        hhm_feature = np.load(data_path + "hmm/" + ID + '.npy')
        dssp_feature = np.load(data_path + "dssp/" + ID + '.npy')
...
def load_graph(ID, data_path):
    matrix = np.load(data_path + 'distance_map/' + ID + '.npy').astype(np.float32)
...
def evaluate(model, data_loader):
    ...
    for data in data_loader:
        with torch.no_grad():
            node_features, adjacency_matrix , labels = data
...
def parse_fasta(file_path):
    sequences = [seq for seq in SeqIO.parse(file_path,'fasta')]
    ProtName = []
    y = []
    seqs = []
...
#     protein_id = ProtName[idx]
#     amino_acids = seqs[idx]
#     y_id = y[idx]

#     fg = list(zip([protein_id],[y_id]))
    
#     # Load the data
#     test_dataframe = pd.DataFrame(list(zip(protein_id, y_id)), columns =['ID', 'label'])
